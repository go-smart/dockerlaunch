...
def launchlayer( monkeypatch ):
    ...
    launchlayer = DockerLayer ( allowed_images, logger, script_filename, max_containers, docker_socket, shutdown_timeout )
...
def test_init_ ( monkeypatch , launchlayer ) :
    ...
    launchlayer.__init__ ( random_allowed_images , random_logger , random_script_filename , random_max_containers , random_docker_socket , random_shutdown_timeout )
    #launchlayer._logger.info.assert_called_with("Trying to connect to Docker as %d:%d", os.getuid(), os.getgid())
    launchlayer._docker_client = docker.Client(base_url=launchlayer._docker_socket, version='auto')
...
def test_try_launch ( monkeypatch , launchlayer ):
    ...
    launchlayer._launch.return_value    = (
        ...
        'indir'     ,
        'socket1'           )
    result = launchlayer.try_launch ( random_image , random_data_location , random_update_socket  )
    assert ( result == ( True , {                 
